...
for [Daytona](https://www.daytona.io). Each package in [`packages/`](packages/) is versioned,
released, and published independently; the apps in [`apps/`](apps/) are standalone projects
that aren't published to a registry. Contributions are welcome! ❤️
...
4. **Sign off every commit** with `git commit -s` to comply with the DCO (see
   [Licensing](#licensing)).
5. Develop within the package's own folder — each is self-contained, with its own dependency
   manifest and `LICENSE`. Run that package's lint/build/test before opening the PR.
