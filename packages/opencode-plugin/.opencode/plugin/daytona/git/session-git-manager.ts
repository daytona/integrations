...
import type { GatewayHostKeyPin, HostKeyTrust } from './gateway-host-key'
...
export class SessionGitManager {
  ...
  private async withSshAccess<T>(
    fn: (access: { url: string; token: string; trust: HostKeyTrust }) => Promise<T>,
  ): Promise<T> {
    ...
    const trust = await SessionGitManager.hostKeyTrust()
    const { host, port } = trust.endpoint
    const url = `ssh://${host}${port === 22 ? '' : `:${port}`}${this.repoPath}`
    const sshAccess = await this.sandbox.createSshAccess(10)
    try {
      return await fn({ url, token: sshAccess.token, trust })
    } finally {
  ...
  /** Installed once at plugin load; every transfer resolves host-key trust through it. */
  static useHostKeyPin(pin: GatewayHostKeyPin): void {
    SessionGitManager.pin = pin
  }

  static hostKeyTrust(): Promise<HostKeyTrust> {
  ...
  static async gatewayKnownHostsHost(): Promise<string> {
    return knownHostsHost((await SessionGitManager.hostKeyTrust()).endpoint)
  }
  ...
  async initializeAndSync(pluginCtx?: PluginInput) {
    ...
    try {
      ...
      const pushed = await this.withSshAccess(({ url, token, trust }) =>
        this.hostGit.pushLocalToSandboxRemote(this.remoteName, url, token, trust, this.branch, this.worktree),
      )
      if (pushed) {
  ...
  async autoCommitAndPull(pluginCtx?: PluginInput): Promise<boolean> {
    ...
    try {
      ...
      await this.withSshAccess(({ url, token, trust }) =>
        this.hostGit.pull(this.remoteName, url, token, trust, sandboxBranch, this.worktree, this.localBranch),
      )
      toast.show({
