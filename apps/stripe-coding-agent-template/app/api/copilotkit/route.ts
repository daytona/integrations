 ...
 * The Stripe Projects Daytona provider exports prefixed vars after
 * `stripe projects add daytona/sandbox` + `stripe projects env --pull`:
 *   DAYTONA_SANDBOX_API_KEY, DAYTONA_SANDBOX_API_URL, DAYTONA_SANDBOX_ORGANIZATION_ID
 * The Daytona SDK reads the unprefixed DAYTONA_API_KEY / DAYTONA_API_URL /
 * DAYTONA_ORGANIZATION_ID. We bridge both forms here, so the template runs
 ...
 * OpenRouter exposes an OpenAI-compatible Chat Completions API, so we point the
 * AI SDK's OpenAI client at it. Two things matter:
 *  1. We call `.chat(model)` (NOT the default `openrouter(model)`). The default
 *     resolves to the OpenAI *Responses* API (POST /responses), which OpenRouter
 *     does not implement. `.chat()` targets POST /chat/completions, which it does.
 *  2. The key is read from OPENROUTER_API_KEY. If you provision OpenRouter via
 *     Stripe Projects, the key is exported under a service-prefixed name
 *     (e.g. OPENROUTER_API_API_KEY); resolveOpenRouterKey() picks that up too.
 */
function resolveOpenRouterKey(): string | undefined {
  if (process.env.OPENROUTER_API_KEY) return process.env.OPENROUTER_API_KEY
  const prefixed = Object.keys(process.env).find((name) => /^OPENROUTER_.*API_KEY$/.test(name))
...
const agent = new BuiltInAgent({
  // `.chat()` → OpenRouter's OpenAI-compatible /chat/completions endpoint.
...
// The Daytona SDK needs Node APIs — pin this route to the Node.js runtime (not Edge).
export const runtime = 'nodejs'

// The agent loop (up to maxSteps tool calls) runs server-side inside this request.
// 300s is allowed on Hobby and Pro with Fluid Compute (Vercel's default); Pro/
// Enterprise can extend further. Responses stream, so most builds finish sooner.
